# 100 Days of Python

My solutions and projects from Angela Yu's **100 Days of Python Bootcamp**.

## About

This repository contains my progress through the 100 Days of Python course. The projects cover Python fundamentals, object-oriented programming, file handling, APIs, GUI development, automation, and data processing.


---

## Projects

### Beginner Projects

- Day 01 - Band Name Generator
- Day 02 - Tip Calculator
- Day 03 - Treasure Island
- Day 04 - Rock Paper Scissors
- Day 05 - Password Generator
- Day 06 - Escaping the Maze
- Day 07 - Hangman
- Day 08 - Caesar Cipher
- Day 09 - Secret Auction
- Day 10 - Calculator
- Day 11 - Blackjack
- Day 12 - Number Guessing Game
- Day 13 - Debugging
- Day 14 - Higher Lower Game

### Intermediate Projects

- Day 15 - Coffee Machine
- Day 16 - OOP Coffee Machine
- Day 17 - Quiz Game
- Day 18 - Turtle Graphics
- Day 19 - Turtle Race
- Day 20 - Snake Game (Part 1)
- Day 21 - Snake Game (Part 2)
- Day 22 - Pong Game
- Day 23 - Turtle Crossing Game
- Day 24 - Mail Merge and File Handling
- Day 25 - US States Game
- Day 26 - NATO Alphabet Project
- Day 27 - Tkinter Miles to KM Converter
- Day 28 - Pomodoro App
- Day 29 - Password Manager
- Day 30 - Password Manager v2
- Day 31 - Flash Card App
- Day 32 - Automated Birthday Wisher
- Day 33 - ISS Overhead Notifier
- Day 34 - Quiz App v2

### API & Automation Projects

- Day 35 - Rain Alert
- Day 36 - Stock Trading News Alert
- Day 37 - Habit Tracker
- Day 38 - Workout Tracker
- Day 39 - Flight Deals
- Day 40 - Flight Club
- Day 41 - Web Foundation Project

---

## Featured Projects

### Snake Game
Built using Python Turtle Graphics and Object-Oriented Programming.

### Pong Game
Classic Pong implementation using Turtle Graphics.

### US States Game
Interactive geography game using Pandas and CSV data.

### Pomodoro App
Productivity timer built with Tkinter GUI.

### Password Manager
Password storage and generation application.

### Flash Card App
Language learning application using Tkinter.

### Workout Tracker
Tracks workouts using external APIs.

### Flight Club
Flight price monitoring application using APIs and email notifications.

---

## Technologies Used

- Python
- Object-Oriented Programming (OOP)
- Tkinter
- Turtle Graphics
- APIs
- Pandas
- CSV & JSON Handling
- File Management
- Email Automation

---

## Course

Based on Angela Yu's **100 Days of Python Bootcamp**.

---
